...
import PartialPayinCard from '~/components/molecules/guardians/PartialPayinCard';
...
import { DeletePendingOrderOptions } from '~/components/atoms/guardians/DeletePendingOrderOptions';
...
const CardTicket = ({ payment, onDeleted, dependents, disabled }: CardTicketProps) => {
  ...
  const { mutateAsync, isLoading: isLoadingReportAsPaid } = api.payin.reportAsPaid.useMutation({
    onSuccess() {
      queryClient.invalidateQueries(['payments', selectedSchoolId ?? '']);
    },
  });
  ...
  const onClickIWantToPay = async () => {
    try {
      setIsLoading(true);
      const res = await ApiClient.deletePendingPayment(session?.token ?? '', selectedSchoolId ?? '', payment.id);
      if (res.status === 204) {
    ...
    >
      ...
      <div className="self-stretch px-[26px] py-5 border-b border-zinc-300">
        ...
        <div className="inline-flex items-center justify-between w-full font-semibold">
          ...
          <span className="text-lg text-right text-blue-700">{formatPrice(totalAmount, payment?.total_currency)}</span>
        </div>
      </div>
      {reportAsPaid ? (
        ...
        <div className="flex flex-col items-center self-stretch justify-center gap-4 px-[26px] py-5">
          ...
          {isExpiredTicket ? (
            <LoadingButton
              ...
              loading={isLoading || isLoadingReportAsPaid || disabled}
              disabled={isLoading || isLoadingReportAsPaid || disabled}
            >
...
function Pendings() {
  ...
  const selectedSchoolId = useSelectedSchoolId();
  const schoolId = selectedSchoolId ?? session?.user.schools[0]?.id ?? '';
  ...
  const { data: fulfillments, isLoading: isLoadingFulfillments } = api.fulfillment.getFulfillments.useQuery(
    {
      schoolId,
      status: [StatusDc1Enum.PARTIAL_PAID],
    },
    {
      retry: 3,
    }
  ...
  if (isLoadingFulfillments || isLoadingPayins)
    return (
  ...
  return (
    <>
      ...
      />
      {payments?.length || fulfillments?.length ? (
        <>
          ...
          <div className="flex flex-col p-5 space-y-6" data-testid="payments-list">
            {fulfillments?.map((fulfillment) => (
              <PartialPayinCard
                key={fulfillment?.id}
                title={fulfillment?.order_name}
                currency={fulfillment?.payins[0]?.total_currency}
                partialPayins={fulfillment?.payins}
                totalAmount={fulfillment?.final_amount}
                pendingAmount={fulfillment?.pending_amount}
              />
            ))}
        ...
        <div className="inset-0 flex flex-col items-center justify-center h-[calc(100vh_-_87px_-_58px_-_24px_-_40px)]">
          ...
          <Link href={`/guardians/${session?.user?.hash}`}>
            <a
              className={cn(
