...
export default function Login() {
  ...
  const manageAuthError = async (authError: any) => {
    if (authError) {
      if (authError.includes('Responsable con hash')) {
        try {
          const res = await ApiClient.shortToLongUrl(guardianHash as string);
          const link = res?.data?.redirect_url;
          if (link) return _router.push(link);
        } catch (error) {
          return setAlert(authError, 'error');
        }
      }
      setAlert(authError, 'error');
    }
  };
  const sendEmail = (guardianHash: string) => {
    setLoading(true);
    ApiClient.loginGuardian(guardianHash)
      .then(({ data }) => {
        if (data.auth_token) {
      ...
      .catch((err) => {
        manageAuthError(err?.response?.data?.error || 'Error al enviar email');
        setLoading(false);
      });
  };

  useEffect(() => {
    manageAuthError(authError);
  }, [authError]);
