...
import { cardValues, creditCardIcon, ErrorFieldMap, kushkiFormIcons } from '~/utils/kushkiCreditCard';
import type { KushkiType } from '~/utils/kushkiCreditCard';
import NFormTextField from '~/components/atoms/guardians/CustomFormTexField';
...
const kushki = async () => await import('~/utils/kushkiCreditCard');
let kushkiInstance: null | KushkiType = null;
function KushkiCreditCard({ session, commissionValues }: PageProps) {
  if ((typeof window !== 'undefined' && kushkiInstance === null) || kushkiInstance instanceof Promise) {
    kushki().then((mod) => (kushkiInstance = mod.kushki));
  }
  ...
  const cardBrandCode = cardData?.card?.type === 'american-express' ? 'AMEX' : 'CREDIT';
  ...
  const expirationMask = '99/99';

  const isLoading = cardData?.card?.type ? false : true;
    ...
    kushkiInstance?.requestToken(
      ...
      async (response) => {
        if (!('code' in response) && Boolean(itemsQuantity)) {
          ...
          try {
            await ApiClient.postKushkiCard(
              session?.user?.id || '',
              storedCheckoutOrders,
              cardData?.card?.type === 'american-express' ? 'AMEX' : 'CREDIT',
              response.token,
              session?.token
            );
            ...
            Sentry.setContext('requestKushkiToken catch Error', {
              message: JSON.stringify(error),
            });
            Sentry.captureEvent({ message: `requestKushkiToken catch ${JSON.stringify(error)}` });
          }
        } else {
  ...
  const showDrawer = (error: unknown) => {
    if (axios.isAxiosError(error)) {
      ...
      if (field !== '-') {
        ...
        if (Object.keys(DrawerOptions).find((key) => key === code)) {
