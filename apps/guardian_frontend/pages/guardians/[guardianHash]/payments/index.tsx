...
import { useSelectedSchool, useSetSchools } from '~/components/molecules/common/AuthGlobal';
...
import { defineTypeForOder } from '~/utils/orders';
...
function Payments() {
  ...
  const isAllHide = Object.values(paymentPreferences ?? {}).every((pref) => !pref.is_active);
  ...
  const { mutateAsync, isLoading: isLoadingMP } = api.mercadopago.checkout.useMutation();
  ...
  const initMPCheckout = async () => {
    ...
    await mutateAsync({
      ...
      items: constructedOrderTuples,
      backUrlsBase: backUrlBase,
    })
      .then((data) => {
    ...
    <div className="flex flex-col h-screen bg-[#F6F5FA]">
      ...
      <section className="flex flex-col flex-1 px-5 py-9">
        ...
        <form className="flex flex-col justify-between flex-1" onSubmit={handleSubmit(onSubmit)}>
          ...
          <LoadingButton
            loading={loading || isLoadingMP}
            disabled={(loading && isLoading) || isLoadingMP || isAllHide}
