...
type FormState =
  | { telephone: string; mail?: never; method: Extract<LoginMethods, 'whatsapp'> }
  | { mail: string; telephone?: never; method: Extract<LoginMethods, 'email'> };

export type LoginMethods = 'email' | 'whatsapp';
...
const Home = () => {
  ...
  const mutation = useMutation({
    mutationFn: (data: FormState) =>
      ApiClient.validateUser({ method: data.method, value: data.mail || data.telephone || '' }),
  ...
  const onSubmit = (data: FormState) => {
    if (schema.safeParse(data).success) {
      mutation.mutate(
        method === 'email' ? data : { method: 'whatsapp', telephone: telephone.number?.toString() || '' }
      );
    } else {
      if (data.mail) {
        trigger('mail');
  ...
  if (method === 'whatsapp' && telephone.valid && errors?.telephone) {
    ...
    <>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-[1fr_1.5fr] grid-rows-[auto_auto] lg:grid-rows-1 min-h-screen bg-gradient-to-t from-[#BEBDFF66] to-[#D3FDFF66] max-h-fit">
        <div className="flex flex-col items-center lg:items-start bg-[#4A5CFF] bg-gradient-mobile bg-right-top lg:bg-gradient-desktop lg:bg-bottom h-fit lg:h-full bg-no-repeat p-10 pb-20 lg:py-[4.6rem] lg:px-[6.5rem] text-white">
          ...
          <div
            className="border border-solid border-white rounded-2xl backdrop-brightness-125 py-9 px-7 hidden items-start gap-5 max-w-[500px] lg:mb-auto"
            aria-hidden="true"
          >
            <div className="w-16 h-16 rounded-full bg-white flex-[1_0_auto] flex items-center justify-center text-3xl">
              <Emoji symbol="👉" />
            </div>
            <div className="flex flex-col text-white">
              <h4 className="text-xl font-bold mb-1.5 mt-0">Pro tip:</h4>
              <span className="text-sm ">
                Si escribes la palabra “Ingresar” al WhatsApp de Cometa{' '}
                <a
                  target="_blank"
                  rel="noopener noreferrer"
                  href={`https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUM}?text=Ingresar`}
                  className="text-white hover:underline"
                >
                  (+52 1 55 3245 2644)
                </a>{' '}
                te llegará tu link de acceso directo al Portal.
              </span>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <Tabs
            ...
            onValueChange={(val) => setValue('method', val as LoginMethods)}
          >
            <TabsTrigger
              value="whatsapp"
          ...
          <Button
            ...
            type="submit"
            disabled={debouncedSubmitting || mutation.isLoading}
          >
          ...
          <a
            className={cn('text-[#4A5CFF] underline text-base mt-10', {
              'lg:mt-20 lg:mb-[47px]': method === 'whatsapp',
            })}
            href={WHAT_LOGIN}
            target="_blank"
            rel="noopener noreferrer"
