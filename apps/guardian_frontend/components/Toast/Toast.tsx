...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      'fixed top-0 z-[100] inset-x-0 mx-auto flex max-h-screen w-full flex-col-reverse p-4 md:max-w-[420px]',
      className
    )}
    {...props}
  />
...
const toastVariants = cva(
  `data-[swipe=move]:transition-none duration-75 border-2 grow-1 group relative pointer-events-auto flex w-full items-center
	justify-between space-x-4 overflow-hidden rounded-[14px] border p-4 pr-8 sm:p-6 sm:pr-8 shadow-lg transition-all 
