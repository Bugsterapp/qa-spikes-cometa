...
const OnboardingGuardianInfo = ({
  ...
  return (
    <div>
      ...
      <DrawerAlert open={Boolean(errors.root?.email) || Boolean(errors.root?.phone)}>
        <DrawerAlertContent>
...
const OnboardingStudentInfo = ({
  ...
  const identifierIsRequiredValidation = student_identifier_is_required
    ? z.string().nonempty('Debes ingresar un CURP').regex(identifierRegex, 'El formato del CURP es incorrecto')
      ...
      <AnimatePresence initial={false} mode="wait" custom={student ? -1 : 1}>
        {(isLoading || Boolean(students.length)) && !student && !openFormStudent && (
          ...
          >
            {isOnboarding ? (
              <article className="text-gray-300 mb-9">
                <h3 className="mb-8 text-2xl font-bold">Confirma los datos de los estudiantes</h3>
                <p className="mb-8 font-light">
...
const BillingInfoResolver = z.object({
  ...
  rfc: z
    .string()
    .nonempty('El RFC es requerido.')
    .refine((data) => data !== 'XAXX010101000', 'Este RFC no es válido'),
  billingName: z.string().nonempty('La razón social es requerida.'),
...
const OnboardingBillingInfo = ({
  ...
  const [showBillingNameHelperText, setShowBillingNameHelperText] = React.useState(false);
          ...
          <Fieldset>
            ...
            >
              <CustomInput
                ...
                onFocus={() => {
                  if (personType !== personTypeDefault) setShowBillingNameHelperText(true);
                }}
                {...register('billingName', {
                  onBlur: () => showBillingNameHelperText && setShowBillingNameHelperText(false),
                })}
