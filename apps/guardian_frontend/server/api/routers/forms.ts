...
export const formsRouter = createTRPCRouter({
  getForm: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      try {
        const response = await AdmissionsServiceClient.getFormApiV1FormsPkGet(
          input.form_id,
        ...
        const response = await AdmissionsServiceClient.getFormsApiV1FormsGet(
          {
            ...
            include_layout: input.includeLayout,
          },
          {
            headers: {
    ...
    .query(async ({ input, ctx }) => {
      try {
        const response = await AdmissionsServiceClient.getAnswersApiV1FormsPkAnswersGet(
          input.form_id,
          { answered_by: input.answered_by, answered_for: input.answered_for },
          {
            headers: {
              Authorization: ctx.session.token,
            },
