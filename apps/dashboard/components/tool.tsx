...
export const ToolHeader = ({ className, type, state, ...props }: ToolHeaderProps) => (
  <CollapsibleTrigger className={cn('flex w-full items-center justify-between gap-4 p-3', className)} {...props}>
    <div className="flex items-center gap-2">
      <WrenchIcon className="size-4 text-gray-500 dark:text-gray-400" />
      <span className="font-medium text-sm">{type}</span>
      {getStatusBadge(state)}
    </div>
    <ChevronDownIcon className="size-4 text-gray-500 transition-transform group-data-[state=open]:rotate-180 dark:text-gray-400" />
  </CollapsibleTrigger>
);
...
export const ToolInput = ({ className, input, ...props }: ToolInputProps) => (
  <div className={cn('space-y-2 overflow-hidden p-4', className)} {...props}>
    <h4 className="font-medium text-gray-500 text-xs uppercase tracking-wide dark:text-gray-400">Parameters</h4>
    <div className="rounded-md bg-gray-100/50 dark:bg-gray-800/50">
