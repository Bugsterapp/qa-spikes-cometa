...
export const TaskItemFile = ({ children, className, ...props }: TaskItemFileProps) => (
  <div
    className={cn(
      'inline-flex items-center gap-1 rounded-md border border-gray-200 bg-gray-100 px-1.5 py-0.5 text-gray-950 text-xs dark:border-gray-800 dark:bg-gray-800 dark:text-gray-50',
      className
    )}
    {...props}
  >
...
export const TaskTrigger = ({ children, className, title, ...props }: TaskTriggerProps) => (
  <CollapsibleTrigger asChild className={cn('group', className)} {...props}>
    {children ?? (
      <div className="flex cursor-pointer items-center gap-2 text-gray-500 hover:text-gray-950 dark:text-gray-400 dark:hover:text-gray-50">
...
export const TaskContent = ({ children, className, ...props }: TaskContentProps) => (
  <CollapsibleContent
    ...
    {...props}
  >
    <div className="mt-4 space-y-2 border-gray-100 border-l-2 pl-4 dark:border-gray-800">{children}</div>
  </CollapsibleContent>
);
