...
import { TabsWrapper as Tabs, useTab } from '/src/components/atoms/Tabs';
...
function ConceptDetail() {
  ...
  const { data: concept, isLoading: isLoadingConceptDetail } = api.schools.schoolsConceptDetail.useQuery(
    {
  ...
  const { data: categories } = api.schools.schoolsConceptsFilters.useQuery(
    { school_id: selectedSchoolId as string },
    {
      refetchOnWindowFocus: false,
      staleTime: 60 * 1000 * 60,
  ...
  const tabsConceptsData = [
    ...
    ...(concept?.optional && flags?.show_variants
      ? [
          {
            value: 'variants',
  ...
  return (
    <div>
      ...
      >
        <div className="sticky z-20 flex flex-col bg-white top-16">
              ...
              {permissions?.can_add_concept_assignment ? (
                <div className="flex gap-3">
                  {flags?.mass_assign_concept ? (
                    <Button onClick={onOpenAssingOrders} className="flex h-10 gap-2" data-testid="assignStudent-btn">
                      <IcStudents />
                      Asignar estudiantes
                    </Button>
                  ) : null}
        ...
        {tab === 'information' && (
          <div className="grid grid-cols-5 gap-6">
            ...
            >
              <div className="space-y-2.5 rounded-2xl p-6 bg-white">
                ...
                <div className="flex items-center mt-6">
                  ...
                  <p className="text-sm font-semibold break-all max-w-[300px]">{concept?.name}</p>
                </div>
                <div className="flex items-center mt-6">
                ...
                <div className="flex items-center mt-6">
                  <p className="text-xs text-[#637381] font-medium w-[25%] mr-3">
                    ¿En dónde se puede pagar el concepto?
                  </p>
                  <div>
            ...
            >
              <div className="space-y-2.5 rounded-2xl p-6 bg-white">
                <p className="border-b-2 pb-1 font-bold text-xs text-[#637381]">ÓRDENES</p>
                {concept?.orders &&
