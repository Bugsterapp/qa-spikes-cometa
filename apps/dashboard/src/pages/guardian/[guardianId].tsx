...
function GuardianPage() {
  ...
  const isWPNotification = !!guardianDetail?.send_whatsapps;
  const isMailNotification = !!guardianDetail?.send_emails;
  ...
  const wpMutation = useMutation({
    mutationFn: async () => {
      await changeNotification('send_whatsapps', !isWPNotification);
    },
    onSuccess: async () => {
  ...
  const mailMutation = useMutation({
    mutationFn: async () => {
      await changeNotification('send_emails', !isMailNotification);
    },
    onSuccess: async () => {
      await utils.guardian.getDetails.invalidate();
      await utils.guardian.getDetails.refetch();
                ...
                <div className="flex">
                  <div className="border-r pr-7">
                    <SwitchWithLabel
                      ...
                      checked={Boolean(guardianDetail?.send_whatsapps)}
                    />
                    <CornerTooltip
                      isOpen={openModalWP}
                  ...
                  <div className="pl-3">
                    <SwitchWithLabel
                      ...
                      checked={Boolean(guardianDetail?.send_emails)}
                    />
                    <CornerTooltip
