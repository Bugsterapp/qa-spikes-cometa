...
import ComplianceCard from '../../components/organisms/dashboard/ComplianceCard';
...
import DelinquentsChart from '../../components/organisms/dashboard/BarChart';
          ...
          <div className="px-6 mr-6">
            ...
            <Grid as="section" columns={['grid-cols-3']} className="gap-4 mb-4">
              {schoolCycles?.length ? (
                ...
                >
                  <Select.Content className="w-full max-w-[164px] outline-none">
                    {schoolCycles?.map((cycle) => (
                      <>
                        {selectedSchoolCycle.includes(cycle.id) ? (
                          <Select.Item className="w-full bg-gray-200 outline-none" value={cycle.id} key={cycle.id}>
                            {cycle.name}
                          </Select.Item>
                        ) : (
                          <Select.Item className="w-full outline-none" value={cycle.id} key={cycle.id}>
                            {cycle.name}
                          </Select.Item>
                        )}
                      </>
