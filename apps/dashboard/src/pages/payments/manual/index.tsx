...
import { BillingStudent, DashboardSchool } from '@cometa/trpc/src/types';
...
import CloseIcon from '/public/assets/icons/ic_close.svg';
import YDivider from '/public/assets/icons/ic_divider.svg';
...
import SuccessIcon from '/public/assets/icons/ic_success.svg';
...
import DatePicker from '/src/components/organisms/dashboard/DatePicker';
import OrderTableForManualPay, { IOrderDetail } from '/src/components/organisms/dashboard/OrderTableForManualPay';
import {
  useSetIdToHighlight,
  useSetTypeOfPayment,
} from '/src/components/organisms/dashboard/OrderTableForPayins/SeePaymentStore';
import RFCDetail, { IUpdateData } from '/src/components/organisms/dashboard/RFCDetail';
import StudentSelectorCard from '/src/components/organisms/dashboard/StudentSelectorCard';
...
const schema = z.object({
  ...
  selectedGuardian: z
    .object({
      ...
      fraud_status: z
        .enum([
          FraudStatusEnum.NoRisk,
          FraudStatusEnum.LowRisk,
          FraudStatusEnum.MediumRisk,
          FraudStatusEnum.HighRisk,
          FraudStatusEnum.UnderInvestigation,
        ])
        .optional(),
    })
    .optional(),
...
interface RenderInvoiceConfigProps {
  ...
  school: DashboardSchool | undefined;
}
  ...
  const updateGuardianMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      setAlertState({
        open: true,
        severity: 'success',
        message: '¡Los datos de RFC se guardaron de manera exitosa!',
      });
    },
      ...
      <>
        ...
        <div className="flex flex-row gap-4 mb-8">
          {students.map((student) => {
            const notCanEmitGeneralPublic =
              student?.billing_guardian === null && school?.can_invoice_to_general_public === false;

            const hasError = paymentErrors.billing_guardian || student.errorRFC || notCanEmitGeneralPublic;
            return (
              <div
                key={student.id}
              ...
              >
                ...
                {notCanEmitGeneralPublic ? (
                  <span className="flex text-[#7A0C2E] mt-2">
                    El colegio no está configurado para emitir facturas a público en general, contacta a tu Account
                    Manager.
                  </span>
                ) : null}
                {hasError && !notCanEmitGeneralPublic && (
                  <button
                    type="button"
                    className={cn(
                      'flex justify-center w-full mt-6 border border-green/48 text-green rounded-lg py-1 px-2.5 active:bg-green/8 disabled:border-gray-500 disabled:text-foreground disabled:cursor-not-allowed disabled:active:bg-transparent',
...
const ItemButton = ({ element }: { element: Record<string, string> }) => (
  <div className="flex flex-col items-start gap-1 font-normal">
    ...
    <div className="flex flex-row items-center gap-2 text-xs text-[#637381]">
      <span>
      ...
      <YDivider />
      {element.state === 'lead' ? (
        <Chip intent="info" className="text-xs">
          Prospecto
        </Chip>
      ) : null}
    </div>
...
function ManualPay() {
  ...
  const { selectedItems, clear } = useSelectionStore<ProjectEnum.DASHBOARD>();
  const cartItems = useCartItems<ProjectEnum.DASHBOARD>();
  ...
  useEffect(() => {
    if (blockedPeriodData?.is_blocked) {
      router.push('/');
    }
  }, [blockedPeriodData, router]);
  ...
  const [partialPayDisabled, setPartialPayDisabled] = useState(false);
  ...
  const { data: bankAccount, isPending: isLoadingBankAccount } = api.manualPayments.bankAccountList.useQuery(
    {
  ...
  const isHighRiskGuardian = selectedGuardian?.fraud_status === FraudStatusEnum.HighRisk;
  const mutation = useMutation({
    mutationKey: ['saveManualPay', selectedGuardian],
    mutationFn: async (parcial: boolean) => saveManualPay(parcial),
    onSuccess: async (response: ManualPaymentResponse) => {
      const studentIds = selectedItems
        .filter((item) => 'student' in item && item.student)
        .map((item) => ('student' in item && item.student ? item.student.id : null))
        .filter(Boolean);

      sendTrackEventWithUserName(TrackEvents.manualPayments.created, {
        payin_id: response?.id,
        amount: response?.total || totalToPartialPay,
        guardian_id: selectedGuardian?.id,
        student_ids: studentIds,
        user_id: user?.id,
        payment_method: selectedPayMethod,
        is_partial: !partialPayDisabled || hasPartialPaymentSelected,
      });
    ...
    onError: async (error: any) => {
      ...
      sendTrackEventWithUserName(TrackEvents.manualPayments.failed, {
        failure_reason: error?.message || 'Unknown error',
        error_code: error?.code || 'manual_payment_error',
        amount: totalToPartialPay,
        guardian_id: selectedGuardian?.id,
        user_id: user?.id,
      });
      ...
      if (
        ...
        const orderWithOutStock = selectedItems.find((orderSelected) => {
          const orderForPay = optionalOrders?.results?.find((order) => order.id === orderSelected.id);

          return orderForPay?.stock?.is_limited && orderForPay?.stock?.quantity === 0;
        });
        if (orderWithOutStock && !isDashboardDependentFulfillment(orderWithOutStock))
          setAlertState({
            severity: 'error',
            open: true,
            message: (
              <>
                El concepto <span className="font-semibold">“{orderWithOutStock?.name}“</span> se ha quedado sin stock.
                Si necesitas puedes modificarlo desde el detalle del concepto .
              </>
            ),
            action: canViewConceptPage
              ? {
                  text: 'Ir al concepto',
                  callback: () => {
                    router.push(`/concepts/${orderWithOutStock.concept.id}`);
                  },
                }
              : undefined,
          });
  ...
  const getStudentsOnSchool = async (search: string) => {
    const studentsOnSchool = await ApiClient.getStudentsOnSchool(selectedSchool?.id || '', search);
    return studentsOnSchool?.results;
  };

  const getPayersOnSchool = async (search: string) => {
    const guardiansOnSchool = await ApiClient.getGuardiansOnSchool(selectedSchool?.id || '', search);
    return guardiansOnSchool?.results;
  };
  ...
  const { data: ComboboxData, isPending: isLoadingCombobox } = useQuery({
    queryKey: ['ComboboxData', debouncedQuery, searchBy],
    queryFn: async () => {
  ...
  const saveManualPay = async (parcial: boolean) => {
    ...
    try {
      const selectedFulfillments = selectedItems.filter((fulfillment) => 'status' in fulfillment);
      const fulfillments = selectedFulfillments.map((fulfillment) => fulfillment.id);
      const fulfillmentOrders = selectedFulfillments.map((fulfillments) => fulfillments.order_id);
      const optionalOrders = cartItems.filter((item) => !fulfillmentOrders.includes(item.order));

      const response = await ApiClient.saveManualPay(
        selectedSchool?.id,
        selectedGuardian?.id,
        payDate.split('T')[0],
        selectedPayMethod,
      ...
      if (response) {
        setId(response?.id);
        setPaymentAlertData({
          correlativeId: response?.correlative_id,
          amount: response?.total,
          paymentDate: response?.paid_date,
          href: `/receipt?school_id=${selectedSchool?.id}&payin_id=${response?.id}&payin_tab=complete`,
        });
      }
  ...
  const onSelectGuardian = (guardian: FormValues['selectedGuardian']) => {
    if (guardian) sendTrackEventWithUserName(Events.manual_payment_guardian_selected, {});
    setValue('selectedGuardian', guardian);
    setOpenChargeBackModal(guardian?.fraud_status === FraudStatusEnum.HighRisk);
  ...
  const onSelectStudent = (student: FormValues['selectedStudent']) => {
    sendTrackEventWithUserName(Events.manual_payment_student_selected, {});
  ...
  return (
    ...
    >
          ...
          {guardian && !isHighRiskGuardian && (
            <>
              ...
              <OrderTableForManualPay
                ...
                partialPayDisabled={partialPayDisabled}
                setPartialPayDisabled={setPartialPayDisabled}
                    ...
                    render={({ field: { onChange, value } }) => (
                      <div className="w-[10rem]">
                        <DatePicker
                          ...
                          minDate={minDate}
                          className="bg-white' focus:ring-[#00AB55] focus:border-[#00AB55]"
                        />
                      </div>
                    )}
                  />
                  <Controller
                    ...
                    render={({ field: { onChange, value } }) => (
                      <div className="flex flex-col">
                        ...
                        >
                          <Select.Content className="flex flex-col rounded-lg min-w-[230px]">
                            {bankAccount?.results &&
              ...
              <div className="mb-8">
                ...
                <div className="flex justify-between items-center mt-2">
                  <CheckBox.Group error={errors.show_comment?.message}>
                    <div className="flex items-center gap-2">
                      <CheckBox.Item
                        id="show_comment"
                        className="w-4 h-4 border-gray-300"
                        checked={watch('show_comment')}
                        disabled={!comment_length}
                        onCheckedChange={(value) => setValue('show_comment', Boolean(value))}
                      />
                      <label htmlFor="show_comment" className="text-xs text-[#637381]">
                        Incluir este comentario en la impresión del recibo.
                      </label>
                    </div>
                  </CheckBox.Group>
                  <div className="text-[#637381] text-xs">{`${comment_length}/150`}</div>
                </div>
              </div>
            ...
            <div className="flex justify-end">
              ...
              >
                <button
                  ...
                  disabled={
                    selectedItems.length === 0 || mutation.isPending || disableByErrorRfc || totalToPartialPay === 0
                  }
      ...
      <Dialog.Root open={!!openConceptsBillables} position="center" classNames="right-16">
        <Dialog.Title>Haz seleccionado conceptos sin facturas</Dialog.Title>
        <Dialog.Description>
          Los siguientes conceptos han sido configurados como sin facturas y no se incluirán en las facturas a emitir.
      ...
      <Dialog.Root open={openChargeBackModal} position="center" classNames="w-[440px]">
        <Dialog.Title>
          <div className="flex flex-col items-center mb-3">
            <Information className="w-14 h-14 text-[#FF4842]" />{' '}
          </div>
          <span className="font-bold text-xl text-[#454D64]">Contracargo pendiente</span>
        </Dialog.Title>
        <Dialog.Description>
          <div className="font-normal text-sm text-[#637381]">
            <p>Actualmente este tutor presenta contracargos</p>
            <p>
              Por favor comuníquese con el tutor para que realice su pago de contracargo a traves del Portal de Pagos de
              Cometa.
            </p>
          </div>
          <div className="flex justify-center">
            <div className="flex items-center justify-around mt-4 px-[10px] py-3 h-10 bg-[#3366FF1A] rounded-lg">
              <div className="flex items-center mr-4">
                <IcMail className="text-[#454D64]" />
                <span className="ml-[6px] text-[#454D64] font-normal text-xs">{selectedGuardian?.email}</span>
              </div>
              <label className="text-[#919EAB] text-base font-medium pl-4 border-l border-l-[#919EAB3D]">
                <div className="flex items-center">
                  <IcPhone />
                  <span className="ml-[6px] text-[#454D64] font-normal text-xs">{selectedGuardian?.phone}</span>
                </div>
              </label>
            </div>
          </div>
        ...
        <Button
          ...
          variant="primary"
          size="tooltip"
          onClick={() => setOpenChargeBackModal(false)}
        >
          Entendido
        </Button>
      </Dialog.Root>
