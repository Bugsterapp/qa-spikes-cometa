...
import YDivider from 'dashboard/public/assets/icons/ic_|.svg';
...
import { useVerifyGuardians } from '/src/hooks/useVerifyGuardians';
...
function ManualPay() {
  ...
  const concepts_not_billable = (selectedItems as any[]).filter((item) => !item.is_billable);
  ...
  const handleButton = () => {
    ...
    if (!selectedBankAccount) {
      targetScroll.scrollIntoView({ behavior: 'smooth' });
      errors = { ...errors, bankAccount: true };
    }
                ...
                <ToastDescription>
                  <div className="flex flex-col items-start justify-center gap-2 pt-6">
                    <div className="flex flex-row items-center justify-center gap-2 pt-4 flex-start">
                      ...
                      <span className="font-semibold text-green text-md">{paymentAlertData?.correlativeId}</span>
                    </div>
                      ...
                      <div className="flex flex-col">
                        <Select
                          ...
                          className={cn('w-[300px] h-[54px] outline-none', {
                            'border-red-500': paymentErrors.bankAccount && !selectedBankAccount,
                          })}
                        ...
                        {paymentErrors.bankAccount && !selectedBankAccount && (
          ...
          {permissions?.can_add_payment && selectedGuardian ? (
            <div className="flex justify-end">
              <Tooltip
                ...
                message={
                  !disablePayButton && !disableByErrorRfc
              ...
              >
                <button
                  ...
                  disabled={selectedItems.length === 0 || mutation.isLoading || disableByErrorRfc}
