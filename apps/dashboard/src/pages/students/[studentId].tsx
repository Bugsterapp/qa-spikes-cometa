...
import { InfoSection } from '/src/components/organisms/dashboard/StudentSections/InfoSection';
import { AccountSection } from '/src/components/organisms/dashboard/StudentSections/AccountSection';
import { ConceptAndScholarshipSection } from '/src/components/organisms/dashboard/StudentSections/ConceptAndScholarshipSection';
import { HeaderSection } from '/src/components/organisms/dashboard/StudentSections/HeaderSection';
import { AdditionalSection } from '/src/components/organisms/dashboard/StudentSections/AdditionalSection';
...
import { StudentScores } from '/src/components/academic/student-scores';

StudentPage.getLayout = function getLayout(page: JSX.Element) {
  return (
    <Layout title="Detalle del estudiante" dashboardVariant="stretch">
      {page}
    </Layout>
  );
};
...
function StudentPage() {
  ...
  const { tab, handleTabChange } = useTab({ studentId });

  const { data: student, isPending: isLoading } = api.students.dashboardSchoolDueOrdersStudentDetail.useQuery(
    { studentId, schoolId: selectedSchool?.id as string },
    { enabled: !!selectedSchool?.id && !!studentId }
  );

  const levelId = student?.section?.level;

  const { data: schoolSteps } = api.admissions.getSchoolSteps.useQuery(
    { schoolId: selectedSchool?.id as string },
    { enabled: !!selectedSchool?.id }
  );
  const schoolStepId = schoolSteps?.find((s) => s.tag === SchoolStepTags.Documents)?.id;
  ...
  const tabComponents: { [key: string]: JSX.Element | null } = {
    info: <InfoSection student={student} isLoading={isLoading} />,
    account: <AccountSection studentId={studentId} />,
    concept: <ConceptAndScholarshipSection student={student} />,
    additional: (
      <AdditionalSection
        studentId={studentId}
        schoolId={selectedSchool?.id as string}
    ...
    scores: <StudentScores studentId={studentId} />,
    documents: (
      <DocumentsSection
  ...
  return (
    <div className="antialiased font-lota">
      <Nav />
      <HeaderSection student={student} />

      <Tabs tab={tab} handleTabChange={handleTabChange}>
        <section className="h-[calc(100vh-214px)] p-8 bg-[#8B93A00A]">{tabComponents[tab] || null}</section>
      </Tabs>
    </div>
  );
}
...
function Nav() {
  ...
  return (
    <nav className="flex items-center px-8 h-14 gap-1 hover:cursor-pointer">
      <Link href={prevPath || '/student'} className="flex items-center gap-1">
        <BackArrow />
...
function Tabs({
  ...
  const showScoresSection = Boolean(selectedSchool?.config_dashboard?.enable_academic_access);
...
function useTab({ studentId }: { studentId: string }) {
  const router = useRouter();
  ...
  function handleTabChange(tab: string) {
    const { tab: _, studentId: __, ...queryParams } = router.query;

    return router.push({
      pathname: `/students/${studentId}`,
      query: tab === 'info' ? queryParams : { ...queryParams, tab },
    });
  }
