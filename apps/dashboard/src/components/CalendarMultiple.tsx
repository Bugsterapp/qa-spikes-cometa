  ...
  return (
    <Popover.Root open={open}>
      ...
      <Popover.Portal>
        <Popover.Content
          align="center"
          className="relative z-40 flex pb-[calc(var(--footer-height)_+_2px)] overflow-hidden transition-transform bg-white divide-x rounded-lg shadow-md data-[state=open]:data-[side=bottom]:animate-slideUpAndFade will-change-[transform,opacity] justify-around"
    ...
    <section className="mx-4 w-[264px] [&:not(:first-child)]:pl-4 [&:not(:first-child)]:ml-0 [&:not(:first-child)]:w-[calc(264px_+_1rem)]">
      ...
      <div className="grid items-center h-8 grid-cols-7 mb-2 gap-y-2 justify-items-center text-[#919EAB]">
        {weekDays.map((d, index) => (
          <p className="block w-6 h-6 text-xs text-center" key={index}>
            {d}
          </p>
        ))}
      </div>
