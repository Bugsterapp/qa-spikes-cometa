...
import { FormDiscount, FormValues4, schemaStep4, StepProps } from '../CreateConcept';
...
export function Step4Form({ setData, onNext, onBack, formData }: StepProps<FormValues4>) {
  ...
  const normalizeDiscount = (discount: FormDiscount, isSingleOrder: boolean) => {
    if (discount.name) {
      return {
        ...discount,
        need_up_to_days: discount.up_to_days === '0',
        up_to_days:
          isSingleOrder && !isNaN(Number(discount.up_to_days)) && !discount.need_up_to_days && formData?.due
            ? (() => {
                const dueDate = new Date(formData.due);
                const discountDate = new Date(dueDate);
                discountDate.setDate(dueDate.getDate() - Number(discount.up_to_days));
                return discountDate.toISOString();
              })()
            : String(discount.up_to_days),
    ...
    return {
      ...discount,
      up_to_days: !isSingleOrder ? String(discount.up_to_days) : discount.up_to_days,
      discount_value: String(discount.discount_value),
    };
  };

  const onSubmit = () => {
    const normalizedDiscounts = discountItems.map((discount) =>
      normalizeDiscount(discount, formData?.has_months_to_pay === 'false')
    );
    setData({ early_bird_discounts: normalizedDiscounts });
    onNext();
  };
                  ...
                  <p className="p-1 text-sm font-medium mt-1">
                    ...
                    {needPicker ? (
                      <span className="font-medium">
                        el{' '}
                        {item.need_up_to_days || item.up_to_days === '' || item.up_to_days === '0' ? (
                          ...
                          <div>
                            {formData?.due
                              ? (() => {
                                  try {
                                    const dueDate = new Date(formData.due);
                                    if (!isValid(dueDate)) return 'Fecha incorrecta';

                                    if (isDate(item.up_to_days)) {
                                      return format(new Date(item.up_to_days), 'PPP', { locale: es });
                                    }

                                    const discountDate = new Date(dueDate);
                                    const daysToSubtract =
                                      typeof item.up_to_days === 'string'
                                        ? parseInt(item.up_to_days, 10)
                                        : item.up_to_days;

                                    if (isNaN(daysToSubtract)) return 'Fecha incorrecta';

                                    discountDate.setDate(dueDate.getDate() - daysToSubtract);

                                    if (!isValid(discountDate)) return 'Fecha incorrecta';

                                    return format(discountDate, 'PPP', { locale: es });
                                  } catch (error) {
