...
const schema = z
  .object({
    ...
    year: z.string().regex(/^(19\d{2}|20[01]\d|202[0-3])$/, 'Año valido entre 1900 y 2023'),
    month: z.string().regex(/^(0[1-9]|1[0-2])$/, 'El mes es inválido'),
    day: z.string().regex(/^(0[1-9]|[12]\d|3[01])$/, 'El día es inválido'),
  })
  .refine(
    (data) => {
...
const StudentPersonalDataView = forwardRef<HTMLFormElement, CreateGuardianViewProps>(
  ({ onSubmit, onCancel, student, setIsFormDirty, schoolId }, ref) => {
    ...
    return (
      <div className="flex flex-col h-[100%] justify-between pb-4">
