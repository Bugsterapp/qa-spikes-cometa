              ...
              <div className="px-8 mb-2">
                <div id="order-data">
                  ...
                  <div className="mt-8">
                    ...
                    <div className="grid grid-cols-4 gap-4 mt-4">
                      <Title text="Órdenes incluidas:" />
                      <Value text={payout?.payin_fulfillments?.length || 0} />
                    </div>
                  </div>
                  <div id="payment_amount_detail" className="mt-8 px-4 py-5 bg-[#F5F9FF] rounded-xl">
                    <div className="grid grid-cols-4 gap-4">
                      <AmountTitle
                        text={`Monto recaudado (${payout?.payin_fulfillments?.length} órdenes)`}
                        loading={isLoading}
                      />
                ...
                <div id="fulfillments" className="mt-8">
                  ...
                  <div className="mt-6 mb-6 border rounded-lg border-[#1890FF] border-opacity-[0.9]">
                    {!isLoading &&
                      payout?.payin_fulfillments?.map((payinFulfillment: Record<string, any>) => (
                        <PayinFulfillmentPayout
...
const PayinFulfillmentPayout = ({
  payinFulfillment,
  setSelectedOrder,
}: {
  payinFulfillment: any;
    ...
    >
      <div className="flex items-center col-span-3">
        <label
          className={cn('text-sm text-[#919EAB] font-medium border-r border-[#919EAB3D] pr-2', {
            italic: !payinFulfillment?.correlative_id,
          })}
        >
      ...
      <div className="flex col-span-4">
        <div className="flex flex-col w-full">
          <label className="text-sm font-semibold">{payinFulfillment?.order_name}</label>
          <div className="flex flex-row items-center justify-between">
            <label className="text-xs font-normal">
              {payinFulfillment?.student?.first_name} {payinFulfillment?.student?.last_name}
            </label>
            <label className="text-sm font-semibold">{formatPrice(payinFulfillment?.final_amount, 'MXN')}</label>
          </div>
        </div>
      </div>
    </button>
