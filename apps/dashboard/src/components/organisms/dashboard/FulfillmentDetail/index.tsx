                ...
                <div id="payment_detail" className="mt-11">
                  ...
                  {fulfillment?.has_partial_payins &&
                    ...
                    fulfillment?.payins?.map((partial) => {
                      const partialFulfillment = fulfillment?.payin_fulfillments?.find(
                        (payinFulfillment) => payinFulfillment.payin === partial.id
                      );
                      return (
                        <BoxPartialPayin
                          key={partial.id}
                          partial={partial}
                          payinFulfillment={partialFulfillment}
                          isLoading={isLoading}
                          schoolId={selectedSchool?.id ?? ''}
                          collectedAt={partial?.collected_at || ''}
                          invoiceStatus={invoiceStatus}
                          invoiceStatusI18N={invoiceStatusI18N}
                          failOriginMessage={failOriginMessage}
                          onClick={() =>
                            checkId(trimId(partial.id), 'Ya tienes abierto este pago en un panel anterior.', () =>
                              setSelectedPayin({ id: partial.id, partial: true })
                            )
                          }
                        />
                      );
  ...
  return (
    <div className="p-4 mt-4 border rounded-lg border-info">
      <Container className="mt-2">
        ...
        <div className="col-span-3 w-fit">
          <LinkDetail
            href={`/guardian/${partial.guardian.id}`}
            text={`${partial.guardian.first_name} ${partial.guardian.last_name}`}
            message="Ver detalle de tutor"
            loading={isLoading}
          />
        </div>
      ...
      <div className="flex flex-col mt-4 gap-y-4">
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="flex col-span-2 w-fit">
                <Value text={formatDateShort(partial.paid_date, true)} />
              </div>
            </>
          )}
        </Container>
        <Container>
          <Title text="Medio de pago:" />
          <Value text={payMethods.find((method) => method.id === partial.type)?.label || ''} loading={isLoading} />
        </Container>
        {partial?.invoices[0]?.status === 'failed' ? (
          <>
            <Container>
              <Title text="Estado de factura:" />
              <InvoiceChip
                intent={invoiceStatus?.[partial.invoices[0]?.status] as VariantProps<typeof ChipVariants>['variant']}
              >
                {invoiceStatusI18N?.[partial?.invoices[0]?.status]?.status}
              </InvoiceChip>
            </Container>
            <Container>
              <Title text="" />
              <span className="text-[#637381] text-xs font-normal col-span-3 pr-6">
                {failOriginMessage?.[partial?.invoices[0]?.fail_origin ?? 'cometa']}
              </span>
            </Container>
          </>
        ) : (
          <div className="flex flex-row gap-y-4">
            <Title text="Estado de factura:" />
            <Tooltip message={invoiceStatusI18N?.[partial?.invoices[0]?.status || 'not_requested'].tooltip}>
              <InvoiceChip
                intent={
                  invoiceStatus?.[partial.invoices[0]?.status ?? 'not_requested'] as VariantProps<
                    typeof ChipVariants
                  >['variant']
                }
              >
                {invoiceStatusI18N?.[partial.invoices[0]?.status ?? 'not_requested'].status}
              </InvoiceChip>
            </Tooltip>
          </div>
        )}
        ...
        {partial.correlative_id && (
          <Container>
            <Title text="ID de pago:" />
            <Value text={partial.correlative_id} loading={isLoading} loaderWidth={30} onClick={onClick} />
          </Container>
        )}
        <Container>
          <Title text="Monto pagado:" />
          {isLoading ? <SkeletonText className="w-24" /> : formatPrice(partial.total, 'MXN')}
        </Container>
        <Container>
          <Title text="Recibo:" />
          <LinkToReceipt href={`/receipt?school_id=${schoolId}&payin_id=${partial.id}`} />
        </Container>
      </div>
    </div>
  );
