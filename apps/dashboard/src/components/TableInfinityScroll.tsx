...
type OverlayActionsProps = {
  itemCount: number;
  itemLabel: string;
  actionContent: React.ReactNode | string;
  buttonType: 'red' | 'green';
  onAction: () => void;
};

export const FloatingActionOverlay: React.FC<OverlayActionsProps & { isWide?: boolean }> = ({
  itemCount,
  itemLabel,
  actionContent,
  buttonType,
  ...
  return (
    <div
      className={cn('bg-white shadow-card rounded-xl py-8 px-6 m-2 absolute z-50 bottom-0 left-[35%]', {
        'w-[423px]': isWide,
      })}
    >
      <div
...
export const TableVirtualized = <T extends Record<string, any>>({
  ...
  return (
    <div data-intercom-table>
      <div
        className={cn('h-[2px] bg-green transition-all duration-500 ease-linear animate-pulse', {
          'w-full': isFetching && !isLoading,
          'translate-x-full opacity-0 w-0': !(isFetching && !isLoading),
        })}
      />
