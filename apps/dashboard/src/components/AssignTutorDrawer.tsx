              ...
              <>
                ...
                <Combobox.Options classNames="absolute z-10 flex flex-col w-full px-3 py-4 mt-1 text-base list-none bg-white rounded-md shadow-lg top-full h-fit max-h-60 focus:outline-none sm:text-sm">
                  <div className="overflow-y-scroll">
                    ...
                    {comboboxData?.length === 0 && !isFetching && (
...
export const DrawerView = ({
  ...
  const { data: session } = useSession();
  const { setState, selectedTab, guardianFound, guardian, disabled, setGuardian } = useDrawerStore();
  ...
  const assignGuardianMutation = useMutation({
    ...
    onError: (err) => {
      setDrawerState({ guardian: null, disabled: false, guardianFound: false });
  ...
  const CreateTutorMutation = useMutation({
    ...
    onSuccess: (res: any) => {
      if (res === 'GUARDIAN_CREATED') {
        ...
        if (student) router.push(`/student/detail/${studentId ?? studentCreatedId}`);
      } else {
        setDrawerState({
          guardian: {
            id: res.id,
  ...
  const sendGuardianData = async (data: FormValues) => {
    const parsedValues = {
    ...
    const { data: guardianSearch } = await ApiClient.getGuardiansOnSchool(
      session?.token,
      selectedSchoolId,
      null,
    ...
    if (guardianSearch?.results.length > 0) {
      setDrawerState({
        guardian: guardianSearch?.results[0],
        guardianFound: true,
        disabled: true,
      });
      return;
      ...
      if (studentCreatedId || studentId || router.query.studentId) {
        CreateTutorMutation.mutate({
          ...parsedValues,
          studentId: studentId ?? router.query.studentId?.toString() ?? studentCreatedId,
        });
      }
    }
  };
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        ...
        />
        <div className="flex-1 px-8 py-6 items-center gap-6 overflow-auto h-full">
          ...
          <Dialog.Root
            open={guardianFound}
            position="right"
            onOpenChange={(state) => setState({ guardianFound: state, disabled: state, guardian: null })}
          >
