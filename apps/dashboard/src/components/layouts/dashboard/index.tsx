...
const variants = cva(`py-2 px-4 w-full transition-all`, {
  variants: {
    variant: {
      ...
      default: 'xl:px-9 2xl:px-16 3xl:px-32',
    },
  },
  defaultVariants: {
    variant: 'default',
...
export default function DashboardLayout({
  ...
  useEffect(() => {
    ...
    const components = ['download', 'assign'];
    let newWorkingComponents = [...workingComponents];

    components.forEach((component) => {
      const status = component === 'download' ? statusDownload : statusConceptAssign;
  ...
  }, [statusDownload, statusConceptAssign]);
  return (
