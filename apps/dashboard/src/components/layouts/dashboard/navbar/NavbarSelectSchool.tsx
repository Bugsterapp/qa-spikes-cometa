...
const NavbarSelectSchool = ({ isCollapse }: NavbarSelectSchoolProps) => {
  ...
  const selectItem = (school: School) => {
    ...
    <>
      {schools.length ? (
        <Listbox value={selectedSchool} onChange={selectItem}>
          ...
          <div className={cn('relative w-full h-fit', { 'h-[35%]': open && schools.length > 3 })}>
            {!isCollapse && schools.length > 1 && (
              <Listbox.Options
                className={cn(
                  'cursor-pointer bg-[#F4F6F8] shadow-sm w-full rounded-xl p-4 flex flex-col z-10 absolute transition-all duration-300 ease-in-out',
                  {
                    'max-h-[500px] overflow-y-auto scrollbar': schools.length > 4,
                  }
                )}
              >
                {schools?.map((school) =>
