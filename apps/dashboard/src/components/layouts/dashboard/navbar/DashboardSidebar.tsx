...
const DashboardSidebar = () => {
  ...
  const permissionMapping: Record<string, string> = {
    cobranzas: 'can_view_collections_page',
    morosidad: 'can_view_delinquency_page',
    pagos_recibidos: 'can_view_received_payment_page',
    conceptos: 'can_view_concepts_page',
  };

  const sidebarConfigFiltered = sidebarConfig.filter((item) => {
    const titleKey = item.title.toLowerCase().replace(' ', '_');
    if (titleKey === 'estudiantes') {
      return true;
    }
    const permissionName = permissionMapping[titleKey];
    if (titleKey === 'ingresos') {
      if (
    ...
    return permissionName && permissions[permissionName] === true;
  });

  return (
    <div
      className={twMerge(
        'h-full overflow-auto top-0 left-0 row-span-2 relative ease-in-out duration-500 -mt-14 z-30',
        widthTransition
      )}
    >
      <div
