...
const SheetContent = ({ children, className, disableAutoFocus = false, ...props }: DialogContentPrimitiveProps) => {
  ...
  return (
    <>
      <Dialog.Portal>
        ...
        >
          <Dialog.Overlay className="data-[state=open]:animate-fade-in data-[state=closed]:animate-fade-out bg-black/15 w-full h-full" />
        </div>
        <Dialog.Content
          onOpenAutoFocus={(e) => {
            if (disableAutoFocus) {
          ...
          className={cn(
            'data-[state=open]:animate-slide-in-right transition-all data-[state=closed]:animate-slide-out-right fixed top-0 bottom-0 right-0 z-[var(--z)] max-w-[572px] w-screen h-full bg-white overflow-auto -translate-x-[var(--offset)]',
            className
          )}
          style={
            {
