...
const ApiClient = {
  ...
  getIncomesPayouts(token, schoolId, page, startDate, endDate, paramsFilter = {}) {
    const params = {
      ...
      start_date: startDate,
      end_date: endDate,
      statuses: 'PROCESSING_STATUS,APPROVED_STATUS,SCHEDULED_STATUS',
      ...paramsFilter,
    };
  ...
  generatePayinsFulfillmentsReport(token, schoolId, { guardians, students, startDate, endDate, ...filters }) {
    const params = {
      guardians,
      students,
      start_date: startDate,
    ...
    return middleware(
      _axios.post(
        `dashboard/schools/${schoolId}/payins_fulfillments/xls/${formatParams(params)}`,
        {},
        headerWithAuthorization(token)
      ),
      [middleware401]
  ...
  getSections(token, schoolId) {
    return middleware(_axios.get(`/dashboard/schools/${schoolId}/sections/`, headerWithAuthorization(token)), [
      middleware401,
    ]);
  },
  ...
  getGuardiansOnSchool(token, schoolId, search) {
    if (search) {
      const params = { search };
      return middleware(
