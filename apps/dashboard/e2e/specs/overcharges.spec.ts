...
import { goto, dashboardLogin } from '../../../e2e/helpers/commons';
...
test('Colegio agrega recargo a orden de pago mensual luego lo elimina. @test', async ({ page }) => {
  ...
  await page.getByRole('cell', { name: 'PP Inscrip 2H 23-24 - Febrero, 2023' }).locator('div').click();
...
test('Colegio no puede eliminar recargo a orden de pago mensual con pago parcial registrado. @test', async ({
  page,
}) => {
  //Given Existe un estudiante creado
  ...
  await incomePage.partialPaymentAmountTxt.fill('3000');
