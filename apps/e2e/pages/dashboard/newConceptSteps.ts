...
export class NewConceptSteps {
  ...
  readonly monthlyReinscriptionOpt: Locator;
  readonly setupPeriodicRestrictionsNoRadio: Locator;
  readonly setupPeriodicRestrictionsYesRadio: Locator;
  readonly fiscalEntityList: Locator;

  constructor(page: Page) {
    this.page = page;
    this.conteptTypeList = page.getByTestId('conceptType');
    this.fiscalEntityList = page.getByTestId('selectFiscalEntity-combo');
    this.monthlySchollarshipOpt = page.getByTestId('Colegiatura / Mensualidad');
    this.monthlyReinscriptionOpt = page.getByTestId('Reinscripción');
    this.schoolCicleList = page.getByTestId('Selecciona un ciclo escolar');
    this.twentyFourCicle = page.getByTestId('Ciclo 2023-2024');
    this.currentCycle = page
      .locator('[data-testid]')
      .filter({
        has: page.locator('div', { hasText: 'Ciclo actual' }),
      })
      .first();
    ...
    this.setupPeriodicRestrictionsYesRadio = page.getByTestId('setupPeriodicRestrictions-yes-radio');
    this.setupPeriodicRestrictionsNoRadio = page.getByTestId('setupPeriodicRestrictions-no-radio');
  }
}
