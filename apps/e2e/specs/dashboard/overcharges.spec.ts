...
import { addOrdersToStudent, getConceptListBySchoolId } from '../../helpers/concepts';
...
test.beforeEach(async ({ page }) => {
  ...
  await loginPage.studentsBtn.click();
  await page.waitForResponse(/\/api\/trpc\/schools\.schoolsCycles,students\.studentsInscriptionsDetail\?batch.+/);
  await expect(studentsPage.studentsTxt).toBeVisible({ timeout: 30000 });
  await page.getByTestId('schoolCycle-combobox').nth(1).click();
  await page.getByRole('option', { name: 'Todos' }).click();
  await page.waitForResponse(/\/students\.dashboardSchoolDueOrdersStudents,schools\.schoolsResume\?batch.+/);
});
